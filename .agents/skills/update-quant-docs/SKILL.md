...
description: >
  ...
  3개 문서 파일(AGENTS.md, domain-logic.md, ui-structure.md)을 최신 상태로 동기화한다.
  다음 상황에서 반드시 사용: 기능 추가/변경 후 "문서 업데이트", "rules 업데이트",
  "AGENTS.md 업데이트", 심볼 추가, 로직 변경, UI 구조 변경, 상태 필드 추가/삭제.
...
| 파일 | 경로 | 담당 내용 |
|------|------|----------|
| AGENTS.md | `/Users/hajun/Trading/quant/AGENTS.md` | 프로젝트 개요, 아키텍처, 실행법, 주요 패턴 |
| domain-logic.md | `/Users/hajun/Trading/quant/.Codex/rules/domain-logic.md` | 별지점 공식, T값 규칙, 매수/매도 구조, 리버스모드 |
| ui-structure.md | `/Users/hajun/Trading/quant/.Codex/rules/ui-structure.md` | index.html JS 패턴, localStorage 키, state 필드, 탭 구조 |
...
```
✓ AGENTS.md — HYNIX2X 심볼 아키텍처 설명에 추가
...
- `docs/README.md`도 AGENTS.md에 명시된 대로 함께 업데이트 필요 여부 확인
